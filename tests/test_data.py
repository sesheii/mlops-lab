...
def test_data_schema():
    ...
    assert required_columns.issubset(
        df_train.columns
    ), "У train датасеті бракує необхідних колонок"
    assert required_columns.issubset(
        df_test.columns
    ), "У test датасеті бракує необхідних колонок"
...
def test_no_missing_values():
    ...
    ), "Train датасет містить порожні значення (NaN)"
    assert (
        df_test.isnull().sum().sum() == 0
    ), "Test датасет містить порожні значення (NaN)"
...
def test_target_classes():
    ...
    assert (
        unique_classes == 2
    ), f"Очікувалось 2 класи у 'target', але знайдено: {unique_classes}"

    class_counts = df_train["target"].value_counts(normalize=True)
    assert (
        class_counts.min() > 0.1
    ), "Сильний дисбаланс класів (менше 10% для одного з класів)"
